...
/* ══════════════════════════════════════════════════════════════
   SOLUCIÓN SIDEBAR PERMANENTE — solo CSS, sin JavaScript
   
   Estrategia: ocultar todos los controles que permiten colapsar
   el sidebar, y forzar que siempre esté visible con min-width.
   Funciona en HF Spaces, Streamlit Cloud y mobile.
   ══════════════════════════════════════════════════════════════ */

/* 1. Ocultar TODOS los botones de colapso de Streamlit */
...
/* 2. Forzar el sidebar siempre abierto y visible */
section[data-testid="stSidebar"] {
...
/* 3. En mobile: sidebar ocupa toda la pantalla cuando está "abierto" */
@media (max-width: 768px) {
    ...
    /* En mobile mostrar instrucción para cerrar */
    section[data-testid="stSidebar"]::after {
        content: "▼  Deslizá hacia abajo para ver los gráficos";
        display: block;
        text-align: center;
        padding: 12px;
        font-family: 'Share Tech Mono', monospace;
...
SESSION_LABELS = {
    ...
    "Q":   "Qualifying",
    "R":   "Race",
    # ── Fin de semana Sprint (formato desde 2023) ──────────────────────────
    # FP1 → Sprint Qualifying → Sprint Race → Qualifying → Race
    "SQ":  "Sprint Qualifying",   # clasificación al sprint (antes: Sprint Shootout)
    "S":   "Sprint Race",         # la carrera sprint corta
}

# Sesiones disponibles por tipo de fin de semana
# FastF1 devuelve EventFormat: "conventional" | "sprint" | "sprint_shootout" | "testing"
SPRINT_SESSIONS      = ["FP1", "SQ", "S", "Q", "R"]
TRADITIONAL_SESSIONS = ["FP1", "FP2", "FP3", "Q", "R"]

def get_session_list(year: int, gp: str) -> list:
    """
    Devuelve la lista de claves de sesión disponibles para este GP.
    Detecta automáticamente si es fin de semana sprint o convencional.
    """
    try:
        schedule = fastf1.get_event_schedule(year, include_testing=False)
        ev = schedule[schedule["EventName"] == gp]
        if ev.empty:
            return TRADITIONAL_SESSIONS
        fmt = str(ev["EventFormat"].values[0]).lower()
        if "sprint" in fmt:
...
def render_sidebar():
    with st.sidebar:
        ...
        # Detectar sesiones disponibles para este GP (sprint vs convencional)
        available_sessions = get_session_list(year, gp_name)

        # ── Modo de comparación ──────────────────────────────────────────────
        st.markdown("""
        <div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:#566A7F;
                    letter-spacing:1.5px;text-transform:uppercase;margin:10px 0 4px">
            MODO DE COMPARACIÓN
        </div>""", unsafe_allow_html=True)

        compare_mode = st.radio(
            "modo",
            options=["same_session", "cross_session"],
            format_func=lambda x: (
                "👥  2 pilotos — misma sesión"
                if x == "same_session"
                else "🔄  1 piloto — entre sesiones"
            ),
        ...
        # Badge sprint weekend
        if available_sessions == SPRINT_SESSIONS:
            st.markdown("""
            <div style="font-family:'Share Tech Mono',monospace;font-size:9px;
                        color:#FF87BC;background:#1A0012;border:1px solid #FF87BC30;
                        padding:4px 10px;border-radius:3px;margin-bottom:8px">
                🏎 FIN DE SEMANA SPRINT<br>
                <span style="color:#566A7F">FP1 · Sprint Q · Sprint · Q · Race</span>
            </div>""", unsafe_allow_html=True)

        if compare_mode == "same_session":
            # ── Modo normal: misma sesión, dos pilotos ───────────────────────
            sess = st.selectbox(
                "SESSION",
            ...
            c1, c2 = st.columns(2)
            with c1:
                d1 = st.selectbox("REF DRIVER", drivers, index=0)
            with c2:
                d2 = st.selectbox("COMP DRIVER", drivers,
                                  index=min(1, len(drivers)-1))

        else:
            # ── Modo cruzado: mismo piloto, dos sesiones ─────────────────────
            st.markdown("""
            <div style="font-family:'Share Tech Mono',monospace;font-size:9px;
                        color:#FFD700;letter-spacing:1px;margin-bottom:6px">
                ⚡ Comparando el mismo piloto en 2 sesiones distintas
            </div>""", unsafe_allow_html=True)

            st.markdown("""<div style="font-family:'Share Tech Mono',monospace;
                font-size:9px;color:#E8002D;letter-spacing:1px;margin-bottom:2px">
                🔴 SESIÓN DE REFERENCIA</div>""", unsafe_allow_html=True)
            sess = st.selectbox(
                "Sesión REF",
                options=available_sessions,
                format_func=lambda x: SESSION_LABELS.get(x, x),
            ...
            st.markdown("""<div style="font-family:'Share Tech Mono',monospace;
                font-size:9px;color:#00D4FF;letter-spacing:1px;margin-bottom:2px;
                margin-top:8px">
                🔵 SESIÓN DE COMPARACIÓN</div>""", unsafe_allow_html=True)
            sess2 = st.selectbox(
                "Sesión COMP",
                options=available_sessions,
                format_func=lambda x: SESSION_LABELS.get(x, x),
                key="sess_comp",
                index=min(2, len(available_sessions)-1),
            )
            ...
            d1 = st.selectbox("PILOTO", drivers, index=0)
            d2 = d1
    ...
    return year, gp_name, sess, sess2, d1, d2, n_pts, bthr, bmin, show_raw, load_btn
...
def build_standings(session, session_label: str) -> tuple:
    ...
    is_race = session_label in ("Race", "Sprint Race")
...
def main():
    ...
    (year, gp_name, sess, sess2, driver1, driver2, n_pts,
     bthr, bmin, show_raw, load_btn) = render_sidebar()

    cross_session = (sess != sess2)   # True = mismo piloto, dos sesiones
    ...
    # ── Load ──────────────────────────────────────────────────────────────────
    if load_btn:
        # En modo same_session no se permite el mismo piloto
        if driver1 == driver2 and not cross_session:
            st.warning("Seleccioná dos pilotos diferentes, o usá el modo 'entre sesiones'.")
            return
        ...
        try:
            # ── Cargar sesión(es) ─────────────────────────────────────────────
            lbl1 = SESSION_LABELS.get(sess,  sess)
            lbl2 = SESSION_LABELS.get(sess2, sess2)

            if cross_session:
                status(f"Cargando {lbl1} y {lbl2} — {driver1}…", 10)
                session1 = load_session(year, gp_name, sess)
                session2 = load_session(year, gp_name, sess2)
                # En modo cruzado las etiquetas incluyen la sesión
                label1 = f"{driver1} ({lbl1})"
                label2 = f"{driver1} ({lbl2})"
            else:
                status(f"Loading {year} {gp_name} — {lbl1}…", 10)
                session1 = load_session(year, gp_name, sess)
                session2 = session1
                label1 = driver1
                label2 = driver2

            status(f"Extrayendo vuelta más rápida…", 35)
            r1 = get_fastest_lap(session1, driver1)
            r2 = get_fastest_lap(session2, driver2)
            lap1, tel1, pos1 = (r1 if len(r1)==3 else (*r1, None))
            lap2, tel2, pos2 = (r2 if len(r2)==3 else (*r2, None))

            if tel1 is None:
                st.error(f"Sin telemetría para {driver1} en {lbl1}."); return
            if tel2 is None:
                st.error(f"Sin telemetría para {driver2} en {lbl2}."); return
            ...
            try:
                ...
                all2 = get_all_laps(session2, driver2)
            except Exception:
                all1 = pd.DataFrame(); all2 = pd.DataFrame()

            status("Loading race pace (quick laps)…", 90)
            try:
                ql1 = get_quick_laps(session1, driver1)
                ql2 = get_quick_laps(session2, driver2)
            except Exception:
                ql1 = pd.DataFrame(); ql2 = pd.DataFrame()

            tc1 = get_team_color(session1, driver1, D1)
            tc2 = get_team_color(session2, driver2, D2)
            ...
            session_key2 = f"{year}||{gp_name}||{sess2}"
            ...
            status(
                f"✓  {label1} vs {label2} — "
                f"{len(dist)} pts · {len(bz1)} BZ · {len(bz2)} BZ",
                100, done=True)
    ...
    d1, d2 = d["driver1"], d["driver2"]
    is_cross = d.get("cross_session", False)

    # Context bar
    mode_badge = (
        '<span style="font-family:Share Tech Mono,monospace;font-size:9px;'
        'color:#FFD700;background:#1A1200;border:1px solid #FFD70030;'
        'padding:2px 8px;border-radius:2px;margin-left:4px">🔄 ENTRE SESIONES</span>'
        if is_cross else ""
    )
    st.markdown(f"""
    ...
    # ── TAB 1: Telemetría unificada — velocidad fija + canales elegibles ─────
    with t_tel:
        CHANNELS = {
            "Δ Delta Time":  "delta",
            "Throttle %":    "throttle",
        ...
        st.markdown('''
        <div style="font-family:Share Tech Mono,monospace;font-size:9px;
                    color:#566A7F;letter-spacing:1.5px;margin:8px 0 4px">
            CANALES — velocidad siempre arriba · elegí los canales adicionales
        </div>''', unsafe_allow_html=True)

        selected_ch = st.multiselect(
            label="canales",
            options=list(CHANNELS.keys()),
            default=["Δ Delta Time", "Throttle %"],
            label_visibility="collapsed",
            key="tel_channels",
        )

        n_extra = len(selected_ch)
        n_rows  = 1 + n_extra
        ...
        row_heights_map = {
            ...
            2: [0.55, 0.45],
            3: [0.44, 0.28, 0.28],
            4: [0.38, 0.22, 0.22, 0.18],
            5: [0.32, 0.18, 0.18, 0.16, 0.16],
        }
        if n_rows in row_heights_map:
            row_h = row_heights_map[n_rows]
        else:
            base = 0.30; rest = (1.0 - base) / max(n_extra, 1)
            row_h = [base] + [rest] * n_extra

        subplot_titles = ["VELOCIDAD (km/h)"] + [ch.upper() for ch in selected_ch]
        ...
        # Fila 1: Velocidad (siempre)
        for tel, name, col in [(d["t1"], d1, D1), (d["t2"], d2, D2)]:
            fig_tel.add_trace(go.Scatter(
                x=d["dist"], y=tel["Speed"], mode="lines",
                line=dict(color=col, width=2.2), name=name,
                hovertemplate=f"<b>{name}</b> %{{y:.0f}} km/h<extra></extra>",
            ), row=1, col=1)

        # Canales adicionales
        for r_idx, ch_name in enumerate(selected_ch, start=2):
            ch_key = CHANNELS[ch_name]
            if ch_key == "delta":
                pos_d = np.where(d["delta"] >= 0, d["delta"], np.nan)
                neg_d = np.where(d["delta"] <  0, d["delta"], np.nan)
                fig_tel.add_trace(go.Scatter(
                    x=d["dist"], y=pos_d, mode="lines",
                    line=dict(color=GREEN, width=2),
                    name=f"▲ {d1} gana", showlegend=False,
                    fill="tozeroy", fillcolor="rgba(57,211,83,0.10)",
                    hovertemplate="Δ %{y:+.3f}s<extra></extra>",
                ), row=r_idx, col=1)
                fig_tel.add_trace(go.Scatter(
                    x=d["dist"], y=neg_d, mode="lines",
                    line=dict(color=D1, width=2),
                    name=f"▼ {d2} gana", showlegend=False,
                    fill="tozeroy", fillcolor="rgba(232,0,45,0.10)",
                    hovertemplate="Δ %{y:+.3f}s<extra></extra>",
                ), row=r_idx, col=1)
                fig_tel.add_hline(y=0,
                    line=dict(color="#2E3E50", width=1, dash="dot"),
                    row=r_idx, col=1)
                final_d = float(d["delta"][-1])
                col_end = GREEN if final_d < 0 else D1
                xref_a  = "x" if r_idx == 1 else f"x{r_idx}"
                yref_a  = "y" if r_idx == 1 else f"y{r_idx}"
                fig_tel.add_annotation(
                    x=float(d["dist"][-1]), y=final_d,
                    xref=xref_a, yref=yref_a,
                    text=f"<b>{final_d:+.3f}s</b>",
                    showarrow=True, arrowhead=0, arrowcolor=col_end,
                    font=dict(family="Orbitron, sans-serif", color=col_end, size=10),
                    bgcolor="rgba(7,11,15,0.85)",
                    bordercolor=col_end, borderwidth=1, borderpad=3,
                )
            else:
                data_map = {
                    "throttle": (d["t1"]["Throttle"], d["t2"]["Throttle"], "linear"),
                    "brake":    (d["t1"]["Brake"]*100, d["t2"]["Brake"]*100, "linear"),
                    "gear":     (d["t1"]["nGear"],     d["t2"]["nGear"],     "hv"),
                    "rpm":      (d["t1"].get("RPM", pd.Series(dtype=float)),
                                 d["t2"].get("RPM", pd.Series(dtype=float)), "linear"),
                }
                data1, data2, shape = data_map[ch_key]
                for data, name, col in [(data1, d1, D1), (data2, d2, D2)]:
                    try:
                        fig_tel.add_trace(go.Scatter(
                            x=d["dist"], y=data, mode="lines",
                            line=dict(color=col, width=1.8, shape=shape),
        ...
        if d.get("sector_dists"):
            for sd, scol, slbl in zip(
                d["sector_dists"], ["#FFD700", "#39D353"], ["S2", "S3"]
            ):
                sd = float(sd)
                if sd <= 0 or sd > d["dist"].max() * 1.05:
                    continue
                for row_i in range(1, n_rows + 1):
                    ...
                    fig_tel.add_shape(
                        type="line", x0=sd, x1=sd, y0=0, y1=1,
                        xref=xref_n, yref=yref_n,
                        line=dict(color=scol, width=1.4, dash="dot"),
                    )
                spd_at = float(np.interp(sd, d["dist"], d["t1"]["Speed"].values))
                fig_tel.add_annotation(
                    x=sd, xref="x", y=spd_at + 10, yref="y",
                    text=f"<b>{slbl}</b>", showarrow=False,
                    font=dict(family="Share Tech Mono", color=scol, size=9),
                    bgcolor="rgba(7,11,15,0.82)",
        ...
        for i in range(1, n_rows + 1):
            fig_tel.update_xaxes(**AX, row=i, col=1)
            fig_tel.update_yaxes(**AX, row=i, col=1)
        fig_tel.update_xaxes(title_text="Distancia (m)",
                              title_font=dict(color="#566A7F", size=10),
                              row=n_rows, col=1)
        for ann in fig_tel.layout.annotations:
            ann.update(font=dict(family="Share Tech Mono, monospace",
                                 color="#2E3E50", size=9),
                       xanchor="left", x=0.0)
        ...
        # KPI cards
        delta = d["delta"]; dist_arr = d["dist"]
        gi = int(np.argmax(delta)); li = int(np.argmin(delta))
        final_leader = d1 if delta[-1] > 0 else d2
        c1, c2, c3, c4 = st.columns(4)
        for col_st, label, val, sub, cls in [
            (c1, "FINAL DELTA",    f"{delta[-1]:+.3f}s", f"{final_leader} más rápido", "neu"),
            (c2, f"MAX {d1} GAIN", f"{delta.max():.3f}s", f"at {dist_arr[gi]:.0f}m",  "d2"),
            (c3, f"MAX {d2} GAIN", f"{abs(delta.min()):.3f}s", f"at {dist_arr[li]:.0f}m", "d1"),
            (c4, "AVG SPEED DIFF",
        ...
        # Delta por sector
        if d.get("sector_dists"):
            sd_list = d["sector_dists"]
            def delta_at(td):
                return float(delta[int(np.argmin(np.abs(dist_arr - td)))])
            d_s1  = delta_at(sd_list[0]) if len(sd_list) > 0 else None
            d_s2  = delta_at(sd_list[1]) if len(sd_list) > 1 else None
            d_fin = float(delta[-1])
            def delta_card(label, val, prev):
                if val is None: return ""
                seg  = val - (prev or 0)
                sign = "▲" if seg >= 0 else "▼"
                cls  = "grn" if seg < 0 else "d1"
            ...
            <div class="metrics-row" style="margin-top:10px">
              ...
              <div class="metric-card {'grn' if d_fin < 0 else 'd1'}">
                ...
                <div class="mc-sub">{'▲ ' + d1 if d_fin > 0 else '▼ ' + d2} más rápido</div>
              </div>
            </div>""", unsafe_allow_html=True)
